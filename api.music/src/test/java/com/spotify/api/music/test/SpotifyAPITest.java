...
public class SpotifyAPITest extends BaseAPI {
	...
	String accessToken;
	String clientId = "318c6661bf8747c1a2d65d17b47ed614";
	String clientSecret = "0415a0ab212744d98be552770e56faec";
	...
	*/
	...
	public void getSearchArtist() {
		given().header("Authorization", "Bearer " + accessToken).when()
				.get("/search?q=Taylor Swift&type=artist").then()
				.statusCode(200)
	...
	public void getSearchArtistWithoutType() {
		given().header("Authorization", "Bearer " + accessToken).when()
				.get("/search?q=Taylor Swift").then()
				.statusCode(400)
	...
	public void getPlaylistWithIncorrectId() {
		given().header("Authorization", "Bearer " + accessToken).when()
				.get("/playlists/7mzrIsaAjnXihW3InKjlC3").then()
				.statusCode(404)
	...
	public void getAlbumWithIncorrectId() {
		given().header("Authorization", "Bearer " + accessToken).when()
				.get("/albums/6KImCVD70vtIoJWnq6nGn3").then()
				.statusCode(404)
